...
export const create = async (c) => {
    try {
        ...
        if (!donation) {
            ...
            return c.json(createResponse("Invalid donation", null, false), HttpStatusCodes.BAD_REQUEST);
        }
        const validationErrors = validateDonation(donation);
        if (validationErrors.length > 0) {
            logDonation("VALIDATION_FAILED", { errors: validationErrors });
            return c.json(createResponse(validationErrors.join(", "), null, false), HttpStatusCodes.BAD_REQUEST);
        }
        const orderId = midtransService.generateOrderId();
        const transactionParams = midtransService.createTransactionParams(orderId, donation.donaturName, donation.phoneNumber, donation.donaturMessage, Number(donation.donationAmount));
        ...
        return c.json(createResponse("Donation Created", {
        ...
        return c.json(createResponse("Error creating donation", null, false), HttpStatusCodes.INTERNAL_SERVER_ERROR);
    }
};
export const midtransCallback = async (c) => {
    try {
        const body = await c.req.json();
        const orderId = body.order_id;
        logDonation("CALLBACK_RECEIVED", body, orderId);
        const isSuccess = midtransService.isTransactionSuccessful(body.transaction_status, body.fraud_status);
        if (!isSuccess) {
            logDonation("TRANSACTION_NOT_SUCCESS", {
                status: body.transaction_status,
                fraud: body.fraud_status,
            }, orderId);
            return c.json(createResponse("Transaksi belum berhasil, tidak disimpan", {
                order_id: orderId || "",
        ...
        if (alreadyExists) {
            ...
            return c.json(createResponse("Transaksi sudah pernah disimpan", {
                order_id: orderId || "",
                transaction_status: body.transaction_status || "",
                fraud_status: body.fraud_status || "",
                status_code: body.status_code || "",
        ...
        return c.json(createResponse("Error saving donation", null, false), HttpStatusCodes.INTERNAL_SERVER_ERROR);
    }
};
...
export const generateExcel = async (c) => {
    try {
        ...
        return c.json(createResponse("Error generating Excel file", null, false), HttpStatusCodes.INTERNAL_SERVER_ERROR);
    }
};
export const getTopDonations = async (c) => {
    try {
        ...
        return c.json(createResponse("Top donations retrieved", transformedDonations), HttpStatusCodes.OK);
    }
    catch (error) {
        return c.json(createResponse("Error retrieving top donations", null, false), HttpStatusCodes.INTERNAL_SERVER_ERROR);
    }
};
