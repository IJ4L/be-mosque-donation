export function sanitizeDonationData(donation) {
    if (!donation)
        return null;
    return {
...
export function extractDonationFromCallback(body, deduction) {
...
export function formatPhoneForMidtrans(phoneNumber) {
...
export function createResponse(message, data, success = true) {
    return {
        message,
        data,
        success,
...
export function logDonation(action, data, orderId) {
    const logData = {
        action,
        orderId,
        timestamp: new Date().toISOString(),
