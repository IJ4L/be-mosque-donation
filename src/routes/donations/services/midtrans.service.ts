...
class MidtransService {
  ...
  calculateDeduction(body: MidtransCallbackBody): DeductionCalculation {
    const grossAmount = parseFloat(body.gross_amount || "0");
    const settlementGross = parseFloat(
      body.settlement_gross || body.gross_amount || "0"
    );
  ...
  private calculateFeeByPaymentType(paymentType: string, amount: number): number {
